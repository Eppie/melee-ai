...
class ImitationConfig(BaseModel):
    ...
    strategy: Literal[
        ...
        description=(
            ...
            "RECOMMENDED: 'value_advantage' - focuses on frames where expert did better than model predicted, "
            "which adapts as your model improves and naturally focuses on harder examples."
        ),
    )
    value_k: float = Field(
        default=1.4,
    ...
    hybrid_strategies: list[str] = Field(
        default_factory=lambda: ["value_weighted", "value_filter", "value_advantage"],
        description=(
