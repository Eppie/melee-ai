...
def compute_log_probs(
    ...
    for head, logits in action_logits.items():
        if torch.isnan(logits).any():
            print(
                f"  [compute_log_probs] NaN detected in {head} logits! count={torch.isnan(logits).sum()}"
            )
    for head, actions in actions_taken.items():
        if torch.isnan(actions).any():
            print(
                f"  [compute_log_probs] NaN detected in {head} actions! count={torch.isnan(actions).sum()}"
            )
    ...
    for head in ["main_stick", "c_stick", "shoulder"]:
        if head in action_logits:
            ...
            if not torch.equal(logits, logits_orig):
                n_clamped = (
                    ((logits_orig < -10000.0) | (logits_orig > 10000.0)).sum().item()
                )
                print(
                    f"  [{head}] Clamped {n_clamped} logits: "
            ...
            if torch.isnan(log_prob).any():
                print(
                    f"  [{head}] NaN after log_softmax! logits range: [{logits.min():.2f}, {logits.max():.2f}]"
                )
            ...
            if torch.isnan(selected_log_prob_orig).any():
                print(
                    f"  [{head}] NaN after gather! actions range: [{actions.min()}, {actions.max()}]"
                )
            ...
            if not torch.equal(selected_log_prob, selected_log_prob_orig):
                n_clamped = (
                    ((selected_log_prob_orig < -100.0) | (selected_log_prob_orig > 0.0))
                    .sum()
                    .item()
                )
                print(
                    f"  [{head}] Clamped {n_clamped} log_probs: "
    ...
    if "buttons" in action_logits:
        ...
        if not torch.equal(logits, logits_orig):
            ...
            print(
                f"  [buttons] Clamped {n_clamped} logits: "
                f"range [{logits_orig.min():.2f}, {logits_orig.max():.2f}] "
                f"-> [{logits.min():.2f}, {logits.max():.2f}]"
            )
        ...
        if torch.isinf(log_probs_pos).any() or torch.isnan(log_probs_pos).any():
            ...
            print(
                f"    probs_orig min/max: {probs_orig.min():.10f}, {probs_orig.max():.10f}"
            )
            print(
                f"    inf_count: {torch.isinf(log_probs_pos).sum()}, nan_count: {torch.isnan(log_probs_pos).sum()}"
            )
            # Find which indices have issues
            bad_idx = (torch.isinf(log_probs_pos) | torch.isnan(log_probs_pos)).nonzero(
                as_tuple=True
            )
            if len(bad_idx[0]) > 0:
                print(
                    f"    First bad indices (batch, button): {bad_idx[0][0]}, {bad_idx[1][0]}"
                )
                print(f"    probs value: {probs[bad_idx[0][0], bad_idx[1][0]]}")
                print(
                    f"    probs_orig value: {probs_orig[bad_idx[0][0], bad_idx[1][0]]}"
                )

        if torch.isinf(log_probs_neg).any() or torch.isnan(log_probs_neg).any():
            print(f"  [buttons] Inf/NaN in log(1-probs)!")
            print(
                f"    1-probs min/max: {(1-probs).min():.10f}, {(1-probs).max():.10f}"
            )
            print(
                f"    inf_count: {torch.isinf(log_probs_neg).sum()}, nan_count: {torch.isnan(log_probs_neg).sum()}"
            )
    ...
    if torch.isnan(total_log_probs).any():
        print(
            f"  [compute_log_probs] NaN in final total_log_probs! count={torch.isnan(total_log_probs).sum()}"
        )
...
def compute_entropy(action_logits: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    for head in ["main_stick", "c_stick", "shoulder"]:
        if head in action_logits:
            ...
            if torch.isnan(entropy).any() or torch.isinf(entropy).any():
                print(
                    f"  [entropy] NaN/Inf in {head} entropy! nan_count={torch.isnan(entropy).sum()}, inf_count={torch.isinf(entropy).sum()}"
                )
    ...
    if "buttons" in action_logits:
        ...
        if torch.isnan(button_entropy).any() or torch.isinf(button_entropy).any():
            ...
            print(
                f"  [entropy] NaN/Inf in button_entropy (stable path)! "
    ...
    if torch.isnan(total_entropy).any() or torch.isinf(total_entropy).any():
        print(
            f"  [entropy] NaN/Inf in total_entropy! nan_count={torch.isnan(total_entropy).sum()}, inf_count={torch.isinf(total_entropy).sum()}"
        )
...
def compute_ppo_loss(
    ...
    if torch.isnan(old_log_probs).any():
        print(
            f"  [loss] NaN in old_log_probs! count={torch.isnan(old_log_probs).sum()}"
        )
    if torch.isnan(new_log_probs).any():
        print(
            f"  [loss] NaN in new_log_probs! count={torch.isnan(new_log_probs).sum()}"
        )
        print(
            f"  [loss] new_log_probs stats: min={new_log_probs[~torch.isnan(new_log_probs)].min() if (~torch.isnan(new_log_probs)).any() else 'all NaN'}, max={new_log_probs[~torch.isnan(new_log_probs)].max() if (~torch.isnan(new_log_probs)).any() else 'all NaN'}"
        )
    ...
    if not torch.equal(log_ratio, log_ratio_orig):
        ...
        print(
            f"  [ratio] Clamped {n_clamped} log_ratios: "
    ...
    if torch.isnan(ratio).any() or torch.isinf(ratio).any():
        print(
            f"  [loss] NaN/Inf in ratio! nan_count={torch.isnan(ratio).sum()}, inf_count={torch.isinf(ratio).sum()}"
        )

    # Check advantages
    if torch.isnan(advantages).any() or torch.isinf(advantages).any():
        print(
            f"  [loss] NaN/Inf in advantages! nan_count={torch.isnan(advantages).sum()}, inf_count={torch.isinf(advantages).sum()}"
        )
    ...
    if torch.isnan(surr1).any() or torch.isinf(surr1).any():
        print(
            f"  [loss] NaN/Inf in surr1! nan_count={torch.isnan(surr1).sum()}, inf_count={torch.isinf(surr1).sum()}"
        )
    if torch.isnan(surr2).any() or torch.isinf(surr2).any():
        print(
            f"  [loss] NaN/Inf in surr2! nan_count={torch.isnan(surr2).sum()}, inf_count={torch.isinf(surr2).sum()}"
        )
    ...
    if torch.isnan(total_loss) or torch.isinf(total_loss):
        print(
            f"  [loss] NaN/Inf in total_loss! policy_loss={policy_loss}, entropy_loss={entropy_loss}"
        )
...
def compute_value_loss(
    ...
    if torch.isnan(value_pred).any() or torch.isinf(value_pred).any():
        print(
            f"  [value_loss] NaN/Inf in value_pred! nan_count={torch.isnan(value_pred).sum()}, inf_count={torch.isinf(value_pred).sum()}"
        )
    if torch.isnan(value_target).any() or torch.isinf(value_target).any():
        print(
            f"  [value_loss] NaN/Inf in value_target! nan_count={torch.isnan(value_target).sum()}, inf_count={torch.isinf(value_target).sum()}"
        )
    ...
    if value_clip is not None and old_value_pred is not None:
        if torch.isnan(old_value_pred).any() or torch.isinf(old_value_pred).any():
            print(
                f"  [value_loss] NaN/Inf in old_value_pred! nan_count={torch.isnan(old_value_pred).sum()}, inf_count={torch.isinf(old_value_pred).sum()}"
            )
...
def compute_total_ppo_loss(
    ...
    if torch.isnan(policy_loss) or torch.isinf(policy_loss):
        print(
            f"  [total_loss] NaN/Inf in policy_loss after compute! value={policy_loss}"
        )
    ...
    if torch.isnan(total_loss) or torch.isinf(total_loss):
        print(
            f"  [total_loss] NaN/Inf in FINAL total_loss! policy_loss={policy_loss}, value_loss={value_loss}, value_coef={value_coef}"
        )
