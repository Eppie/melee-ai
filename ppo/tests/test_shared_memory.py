...
class TestFrameData:
    ...
    def test_invalid_shape(self):
        ...
        features = np.random.randn(max(FEATURE_DIM - 1, 1)).astype(np.float32)
    ...
    def test_invalid_dtype(self):
        ...
        features = np.random.randn(FEATURE_DIM).astype(np.float64)
...
class TestSharedMemorySlab:
    ...
    def test_create(self, cleanup_shm):
        ...
        # Verify shapes
        assert slab.features.shape == (8, 256, FEATURE_DIM)
...
class TestPinnedStagingBuffer:
    ...
    def test_allocation(self):
        ...
        buffer = PinnedStagingBuffer(num_envs=96, feature_dim=FEATURE_DIM)

        assert buffer.staging.shape == (96, 1, FEATURE_DIM)
    ...
    def test_copy_from_slabs(self, cleanup_shm):
        ...
        buffer = PinnedStagingBuffer(num_envs=12, feature_dim=FEATURE_DIM)
    ...
    def test_get_tensor(self):
        ...
        assert tensor.shape == (96, 1, FEATURE_DIM)
        # Only check pinning on CUDA systems
        if HAS_CUDA:
