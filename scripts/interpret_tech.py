            ...
            events.append(
                {
                    "X": X,
                    "tech_frame_rel": t
                    - start_frame,  # Index in the window where tech happens
...
def perform_ablation(
    model: GPT, colmap: ColumnMap, event: Dict[str, torch.Tensor], device: torch.device
):
    """
    ...
    if lockout_time >= 0:
        X_lockout[
            0, lockout_time : lockout_time + 5, btn_lr_feat
        ] = 1.0  # Hold for 5 frames
    ...
    print(
        f"  Ablation [Sky High]: {prob_sky:.4f}  (Delta: {prob_sky - baseline_prob:+.4f})"
    )
    print(
        f"  Ablation [Safe Act]: {prob_safe:.4f}  (Delta: {prob_safe - baseline_prob:+.4f})"
    )
    if prob_lockout != -1:
        print(
            f"  Ablation [Lockout]:  {prob_lockout:.4f}  (Delta: {prob_lockout - baseline_prob:+.4f})"
        )
    else:
        print(f"  Ablation [Lockout]:  N/A (Window start)")
    print(
        f"  Ablation [Chill]:    {prob_chill:.4f}  (Delta: {prob_chill - baseline_prob:+.4f})"
    )
...
def main():
    ...
    print(
        "We check the probability of an L/R press in the frames immediately preceding the tech."
    )
