...
class EpisodeStatsCollector(StatsCollector):
    ...
    def __init__(self, feature_names: Sequence[str]) -> None:
        ...
        # Winner tracking (based on death states and percent)
        self._p1_wins = 0
        self._p2_wins = 0
        self._draws = 0
    ...
    def process_episode(self, data: np.ndarray, episode: EpisodeInfo) -> None:
        ...
        # enters death animation or goes offscreen
        ...
        if p2_dying and not p1_dying:
            ...
            self._p2_wins += 1
        elif p1_percent[-1] < p2_percent[-1]:
            # Lower percent wins (tiebreaker)
            self._p1_wins += 1
        elif p2_percent[-1] < p1_percent[-1]:
            self._p2_wins += 1
        else:
            # Both dying or equal percent - true draw/timeout
            self._draws += 1
