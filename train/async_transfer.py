...
class PinnedMemoryPool:
    ...
    Usage:
        ...
        tensor.cpu(out=buffer, non_blocking=True)
    ...
    def transfer_scalar_async(
        ...
        if self.transfer_stream is not None:
            with torch.cuda.stream(self.transfer_stream):
                gpu_tensor.cpu(out=buffer, non_blocking=True)
        else:
            gpu_tensor.cpu(out=buffer, non_blocking=True)
...
class DeferredScalarAccumulator:
    ...
    def get_and_reset(self) -> list[float]:
        ...
        stacked.cpu(out=buffer, non_blocking=False)  # Blocking transfer for simplicity
...
class BatchedStatsTransfer:
    ...
    def execute(self, non_blocking: bool = False) -> dict[str, float | list]:
        ...
        if scalars:
            ...
            stacked.cpu(out=buffer, non_blocking=non_blocking)
        ...
        for name, tensor in arrays.items():
            ...
            flat = tensor.flatten()
            flat.cpu(out=buffer[:flat.numel()], non_blocking=non_blocking)
