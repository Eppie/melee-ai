...
def compute_value_based_weights(
    value_targets: Tensor, loss_cfg: "LossConfig"
) -> Tensor:
    """Compute per-frame weights from value targets.

    The weights are normalized, scaled, shifted into a positive range, and finally clipped to
    keep magnitudes bounded.
    """
    if value_targets.ndim not in (2, 3):
        raise ValueError(
            f"value_targets must have ndim 2 or 3, got shape {tuple(value_targets.shape)}"
        )
    ...
    if values.ndim == 3:
        if values.shape[-1] != 1:
            raise ValueError(
                f"value_targets last dimension must be 1 when 3D, got {values.shape}"
            )
    ...
    weights = 1.0 + normalized * float(loss_cfg.value_weight_scale)
    weights = torch.clamp(weights, min=0.1, max=float(loss_cfg.value_weight_clip))

    if weights.shape != values.shape:
        raise ValueError(
            f"value-based weights shape {tuple(weights.shape)} does not match values {tuple(values.shape)}"
        )
