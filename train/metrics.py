...
class MetricsAccumulator:
    ...
    def __init__(self, K_main: int, K_c: int, K_buttons: int, K_shoulder: int, device: torch.device):
        ...
        self.shoulder_label_counts = torch.zeros(K_shoulder, dtype=torch.long, device=device) if K_shoulder > 0 else None
    ...
    def update_stick_metrics(
        ...
        if repeat_baseline is not None and repeat_mask is not None:
            if repeat_mask.any():
                ...
                setattr(self, rep_correct_attr, getattr(self, rep_correct_attr) + rep_correct)
                setattr(self, rep_total_attr, getattr(self, rep_total_attr) + repeat_mask.sum())
    ...
    def get_summary(self) -> Dict[str, float]:
        ...
        # Main stick
        summary["acc_main"] = float(self.main_correct.item()) / max(1, float(self.main_total.item()))
        summary["acc_main_maj"] = float(self.main_maj_correct.item()) / max(1, float(self.main_total.item()))
        if self.main_rep_total.item() > 0:
            summary["acc_main_rep"] = float(self.main_rep_correct.item()) / float(self.main_rep_total.item())
        else:
            summary["acc_main_rep"] = 0.0
        
        # C-stick
        summary["acc_c"] = float(self.c_correct.item()) / max(1, float(self.c_total.item()))
        summary["acc_c_maj"] = float(self.c_maj_correct.item()) / max(1, float(self.c_total.item()))
        if self.c_rep_total.item() > 0:
            summary["acc_c_rep"] = float(self.c_rep_correct.item()) / float(self.c_rep_total.item())
        else:
        ...
        # Macro-averaged
        prec_per_button = np.divide(tp, tp + fp, out=np.zeros_like(tp), where=(tp + fp) > 0)
        rec_per_button = np.divide(tp, tp + fn, out=np.zeros_like(tp), where=(tp + fn) > 0)
        f1_per_button = np.divide(2 * prec_per_button * rec_per_button, prec_per_button + rec_per_button,
                                   out=np.zeros_like(tp), where=(prec_per_button + rec_per_button) > 0)
        f1_macro = f1_per_button.mean()
        
        summary["btn_em"] = float(self.btn_em_correct.item()) / max(1, float(self.btn_total.item()))
        ...
        summary["btn_em_maj"] = float(self.btn_maj_em_correct.item()) / max(1, float(self.btn_total.item()))
        if self.btn_rep_total.item() > 0:
            summary["btn_em_rep"] = float(self.btn_rep_em_correct.item()) / float(self.btn_rep_total.item())
        else:
            summary["btn_em_rep"] = 0.0
        
        # Shoulder
        if self.K_shoulder > 0:
            summary["acc_shoulder"] = float(self.shoulder_correct.item()) / max(1, float(self.shoulder_total.item()))
            summary["acc_shoulder_maj"] = float(self.shoulder_maj_correct.item()) / max(1, float(self.shoulder_total.item()))
...
def compute_confusion_matrix(true_flat: torch.Tensor, pred_flat: torch.Tensor, K: int) -> torch.Tensor:
...
def multilabel_prf(true_labels: torch.Tensor, pred_labels: torch.Tensor) -> Tuple[float, float, float, float, float]:
